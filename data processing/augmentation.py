...
for class_name, sample_indices in sampled.items():
    ...
    for index in sample_indices:
        ...
        if i <= sampled_change:
            transformed_image = gaussian_filter(image, sigma=np.random.uniform(0.05,0.1))
            cv2.imwrite(os.path.join(transformed_path, files[class_name][index]), transformed_image)
        elif i > sampled_change and i <= 2 * sampled_change:
            jpeg_quality = np.random.randint(60,90) # value between 0 and 100, higher value - higher quality
            cv2.imwrite(os.path.join(transformed_path, files[class_name][index]), image, [cv2.IMWRITE_JPEG_QUALITY, jpeg_quality])
        elif i > 2* sampled_change and i <= 3 * sampled_change:
            transformed_image = gaussian_noise(image, 0, np.random.uniform(0.5,1.5))
            cv2.imwrite(os.path.join(transformed_path, files[class_name][index]), transformed_image)
        elif i > 3 * sampled_change and i <= 4 * sampled_change:
            transformed_image = median_filter(image, size=np.random.randint(1,3))
            cv2.imwrite(os.path.join(transformed_path, files[class_name][index]), transformed_image)
            ...
            transformed_image[:, :, 0] = np.random.uniform(0.7,0.9) * transformed_image[:, :, 0]
            ...
            resize = np.random.randint(0.7 * width, 0.9 * width)
            transformed_image = cv2.resize(image, (resize, resize), interpolation = cv2.INTER_AREA)
